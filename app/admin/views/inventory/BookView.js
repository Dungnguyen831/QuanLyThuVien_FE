class BookView {
   ...
   */
  renderBooks(books, authors, categories, publishers) {
    // Thêm publishers vào tham số
  ...
  renderBookDetailToModal(book, authors, categories, publishers) {
    ...
    const author = authors.find(
      (a) => a.id == (book.author_id || book.authorId),
    );
    const category = categories.find(
      (c) => c.id == (book.category_id || book.categoryId),
    );
    const publisher = publishers.find(
      (p) => p.id == (book.publisher_id || book.publisherId),
    );

    // 2. Đổ dữ liệu vào các ID trong HTML

    ImageService.display("detailBookImage", book.imageUrl);
    document.getElementById("detailBookTitle").textContent =
      book.title || "N/A";
  ...
  renderCopiesToModal(copies, shelves = []) {
    ...
    tbody.innerHTML = copies.length
      ? copies
          ...
          .join("")
      : '<tr><td colspan="6" class="text-center">Chưa có bản sao nào</td></tr>';
