class BookController {
  ...
  refreshAllDatalists() {
    ...
    const configs = [
      { id: "authorOptions", data: this.authors },
      { id: "editAuthorOptions", data: this.authors },
      { id: "categoryOptions", data: this.categories },
      { id: "editCategoryOptions", data: this.categories },
      { id: "publisherOptions", data: this.publishers },
      { id: "editPublisherOptions", data: this.publishers },
    ];
    configs.forEach((c) => {
      ...
      if (el)
        el.innerHTML = c.data
          .map(
            (i) =>
              `<option value="${i.id}">${i.id} - ${i.name || i.title}</option>`,
          )
          .join("");
    });
